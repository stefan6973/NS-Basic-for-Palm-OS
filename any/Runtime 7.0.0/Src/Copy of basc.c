...
int codeexec(Word objId, unsigned long offset)
{
  ...
  clearLockedArgs();
  if (codeBlockPtr->id == 50000)
    instr_ctr = 0; //dummy control for root level
  ...
  *s3_de = 0;
  if ((arg != null_ptr) && (arg->max_no_values1 > 1))
  {
    ...
    if (sub == 0) //  means that used array w/o subs
    {
      ...
      *s2 = 0;
      *s3 = 0;
      if (arg->max_no_values2 > 1)
        find_subscript();
      if (arg->max_no_values3 > 1)
        find_subscript();
      goto getOutB;
    }
...
comp_1low:
  if (argum_switch == 1)
    goto comp_1high_set;
comp_1low_set:
  if ((oper == 3) || (oper == 4) || (oper == 2) || (oper == 7) || (oper == 8))
    return (1);
  else
    return (0);
comp_1hi:
  if (argum_switch == 1)
    goto comp_1low_set;
comp_1high_set:
  if ((oper == 3) || (oper == 1) || (oper == 5) || (oper == 9) || (oper == 6))
    return (1);
  else
    return (0);
comp_1_2_equal:
  if ((oper == 0) || (oper == 4) || (oper == 5) || (oper == 7) || (oper == 6))
    return (1);
  else
    return (0);
}
...
test_debug(test);
----*/
  i = (s3)*arg->max_no_values2 * arg->max_no_values1 + (s2)*arg->max_no_values1 + (s1);
                                                         ...
                                                         /*tbl # of a data elem */
{
  ...
  {                                     //06/01/2002
    ...
    return ((struct dataelem *)null_ptr); //06/01/2002
}
/********************************************************************************************/
...
void subFuncXferArgument()
{
  ...
  {
    if (arg2->kind != 's')
      MiscFatalAlert("Mis-matched args");
  }
  else
  {
    if (arg2->kind == 's')
  ...
  if ((arg2->max_no_values1 < 2) && (((arg1->max_no_values1 > 1) && (arg1_sde1 == 0)) || ((arg1->max_no_values2 > 1) && (arg1_sde2 == 0)) || ((arg1->max_no_values3 > 1) && (arg1_sde3 == 0))))
  {
  ...
  if (arg2->max_no_values1 > 1)
  {
    ...
    if (arg2->max_no_values3 > 1)
    {
      ...
      if (arg1_sde1 != 0)
        MiscFatalAlert("Too many levels of subscripting.");
      if ((arg1->max_no_values1 != arg2->max_no_values1) || (arg1->max_no_values2 != arg2->max_no_values2) || (arg1->max_no_values3 != arg2->max_no_values3))
    ...
    else
    {
      if (arg2->max_no_values2 > 1)
      {
        ...
        if (arg1_sde1 == 0)
        {
          if ((arg1->max_no_values1 != arg2->max_no_values1) || (arg1->max_no_values2 != arg2->max_no_values2))
            MiscFatalAlert("Subroutine and calling program array bounds do not match.");
        }
        else
        {
          if ((arg1->max_no_values2 != arg2->max_no_values1) || (arg1->max_no_values3 != arg2->max_no_values2))
            MiscFatalAlert("Subroutine and calling program array bounds do not match.");
        }
        for (kk = 0; kk < arg2->max_no_values1; ++kk)
        {
...
void verb_nextscreen() //nextform   //08142002 (old before RESET option)
{
  ...
  {                                                //08142002
    ...
    evptr->data.generic.datum[2] = 0; //08142002 dont reset to design state
...
void commonSetFocus(FormPtr frmPtr, int the_one)
{
  ...
  {
    if (FrmGetObjectId(frmPtr, i) != the_one)
      continue;
...
void verb_alert()
{
  ...
  if (res != -1)
    DmRemoveResource(VbRscDB, res); //delete alert if there
...
void verb_sound()
{
  ...
  if (romVersion < 0x03000000)
  {
    if (PrefGetPreference(prefGameSoundLevelV20) == slOff)
      return; //prior to v3.0 -- exit if game sound off
  }
  else
  {
    if (PrefGetPreference(prefGameSoundVolume) == 0)
      return; //v3.0 and up -- exit if game sound volume=0
  }
...
void verb_drawline()
{
  ...
  {
    if (pen == 0x01)
      WinEraseLine(*((double *)arg1_val),
                   *((double *)arg2_val),
                   *((double *)arg3_val),
                   *((double *)arg4_val));
    else
      WinDrawGrayLine(*((double *)arg1_val),
                      *((double *)arg2_val),
                      *((double *)arg3_val),
                      *((double *)arg4_val));
  }
...
void verb_drawchars()
{
  ...
  pen = find_char();
  if (pen == 0x00)
    WinDrawChars(arg1_val, StrLen(arg1_val),
                 *((double *)arg2_val), *((double *)arg3_val));
  else
    WinDrawInvertedChars(arg1_val, StrLen(arg1_val),
                         *((double *)arg2_val), *((double *)arg3_val));
}
...
goahead:
  ...
  while (1)
  {
    ...
    else
    {
      ...
      if (newHand == 0)
        OutOfMemory(6101); //inserted 05282003
      newPtr = MiscHandleLock(newHand);
      break;
    }
  }
  ...
  if (err)
    MiscFatalAlert("Internal error-Can't release work record.");
...
void verb_start_function()
{
  ...
  while (1)
  {
    ...
    else
    {
      ...
      newHand = MiscDmNewResource(VbRscDB, (ULong)'fwrk', id, wkl + 4, "rescwk"); //setup a new resource for the new copy
      if (newHand == 0)
        OutOfMemory(6102); //inserted 05282003
      newPtr = MiscHandleLock(newHand);
      break;
    }
  }
...
void verb_dbclose()
{
  ...
  if (res == 0)
    setDbOK(); //added 09242003
  else
    setDbErr(res); //added 09242003
}
//additions 09242003
...
void verb_dbopen()
{
  ...
  if (dbref == 0)
  {
    ...
    {                           //02.20.02 GH
      ...
      setDbRes(3); //opened in read only mode                            //02.20.02 GH
  }
  else
    setDbOK(); //opened in read write mode
...
void verb_dbupdate()
{
  ...
  *((char *)(&kindInt) + 1) = arg3->flags;
  ...
  if (keyCompare(arg3_val, recPtr, kindInt) == 0)
  {
    ...
    if (result == 1)
      goto getOut8; //out of mem      05282003
        ...
        for (ii = 0; ii < arg5->max_no_values1; ++ii)
        {
          for (jj = 0; jj < arg5->max_no_values2; ++jj)
          {
            for (kk = 0; kk < arg5->max_no_values3; ++kk)
            {
              ...
              if (res != 0)
                return (1); //outOfMemory   inserted 05282003
            }
          }
        }
      }
                  ...
                  UInt index)
{
  ...
  *((int *)&theKind) = other;
  ...
  else
  {
    ...
    {
      if (theKind.flags & 0x40)
        keylen = 8;
      else
      {
        if (theKind.flags & 0x20)
          keylen = 4;
        else
        {
          if (theKind.flags & 0x10)
            keylen = 2;
        }
      }
    }
  ...
  *recHand = DmResizeRecord(db->dm, index, keylen + 1); //leave 1 extra null byte  //1/9/2001
  if (*recHand == 0)
    return (1); //outOfMem       //inserted 05282003
...
void verb_initAVariableValue()
{
  ...
  {
    if (arg1->kind != 's')
      DmSet(arg1, DELEN, 8 * arg1->max_no_values1 * arg1->max_no_values2 * arg1->max_no_values3, 0x00);
    else
    {
      ...
      if (hData == 0)
        OutOfMemory(6004); //inserted 05282003
...
void commonSetFieldTextRoutine(FieldPtr controlPtr, Word controlId, char *s)
{
  ...
  if (oldHand)
    MemHandleFree(oldHand); //0803 m2000

  //FldDrawField ((FieldPtr)controlPtr);

  frmPtr = FrmGetActiveForm();
  if (frmPtr == 0)
    return; //added 5.20.2000
...
void verb_getPen()
{
  ...
  if (penDown == true)
    restore_int_val(2, arg3, (double *)arg3_val, arg3_occ1, arg3_occ2, arg3_occ3);
  else
    restore_int_val(3, arg3, (double *)arg3_val, arg3_occ1, arg3_occ2, arg3_occ3);
}
...
void verb_beep()
{
  if (romVersion < 0x03000000)
  {
    if (PrefGetPreference(prefSysSoundLevelV20) == slOff)
      return; //prior to v3.0 -- exit if sound off
  }
  else
  {
    if (PrefGetPreference(prefSysSoundVolume) == 0)
      return; //v3.0 and up -- exit if sound volume=0
  }
...
void verb_serialOpen()
{
  ...
  {
    if (port == 0)
      port = 0x8000;                    //hotSync port           //02.27.02 GH
...
void verb_serialClose()
{
  ...
  {
    if (gOldSerialManager)
      err = SerClose(serRef); //02.27.02 GH
    else
      err = SrmClose(serRef); //02.27.02 GH
  }
  else
    err = serErrNotOpen;
  serRef = 0;
...
void verb_serialSet() //03.04.02 GH - for newSerialManager
{
  ...
  if (StrCompare(in, "stopbits") == 0)
  {
    ...
    if (i == 1)
      ui32 &= ~srmSettingsFlagStopBits1;
    else if (i == 2)
      ui32 |= srmSettingsFlagStopBits2;
    goto setIt;
  }
  ...
  if (StrCompare(in, "xonxoff") == 0)
  {
    ...
    err = SrmControl(serRef, srmCtlGetFlags, &ui32, &size_ui32);
    if (i == 1)
      ui32 |= srmSettingsFlagXonXoffM; //on
    else if (i == 2)
      ui32 &= ~srmSettingsFlagXonXoffM; //off
    goto setIt;
  }
  if (StrCompare(in, "rtsauto") == 0)
  {
    ...
    err = SrmControl(serRef, srmCtlGetFlags, &ui32, &size_ui32);
    if (i == 1)
      ui32 |= srmSettingsFlagRTSAutoM; //on
    else if (i == 2)
      ui32 &= ~srmSettingsFlagRTSAutoM; //off
    goto setIt;
  }
  if (StrCompare(in, "ctsauto") == 0)
  {
    ...
    err = SrmControl(serRef, srmCtlGetFlags, &ui32, &size_ui32);
    if (i == 1)
      ui32 |= srmSettingsFlagCTSAutoM; //on
    else if (i == 2)
      ui32 &= ~srmSettingsFlagCTSAutoM; //off
    goto setIt;
  }
  if (StrCompare(in, "bitsperchar") == 0)
  {
    ...
    if (i == 5)
      ui32 |= srmSettingsFlagBitsPerChar5;
    else if (i == 6)
      ui32 |= srmSettingsFlagBitsPerChar6;
    else if (i == 7)
      ui32 |= srmSettingsFlagBitsPerChar7;
    else if (i == 8)
      ui32 |= srmSettingsFlagBitsPerChar8;
    goto setIt;
  }
...
void verb_serialSetOld()
{
  ...
  if (StrCompare(in, "stopbits") == 0)
  {
    i = basc_integ_value(arg2, (double *)arg2_val);
    if (i == 1)
      s.flags = s.flags & 0xfffe;
    if (i == 2)
      s.flags = s.flags | 0x0001;
    goto setIt;
  }
  if (StrCompare(in, "parity") == 0)
  {
...
void verb_serialSend()
{
  ...
  //SysFatalAlert(debug);
  if (gOldSerialManager)
    SerReceiveFlush(serRef, 0); //flush any response bytes	//02.27.02 GH
  else
    SrmReceiveFlush(serRef, 0); //02.27.02 GH

  if (arg1->kind == 's')
  {
    //send a string
    if (gOldSerialManager)
      actBytes = SerSend(serRef, arg1_val, bytes, &err); //02.27.02 GH
    else
      actBytes = SrmSend(serRef, arg1_val, bytes, &err); //02.27.02 GH
  }
  else
  {
    //send an integer byte array (send only the low order byte of each integer)
    bytes = long_integ_value(arg2, (double *)arg2_val);
    if (bytes > (serialBufferSize))
      bytes = serialBufferSize; //limit to our work buf size(OS needs another 32 chars)
    if (bytes > arg1->max_no_values1)
      bytes = arg1->max_no_values1; //limit to size of array
    //05282003 pRcvBuf=MemHandleLock(MemHandleNew(bytes+1));
    h = MemHandleNew(bytes + 1); //05282003
    if (h == 0)
      OutOfMemory(6056);        //05282003
    ...
    //}
    if (gOldSerialManager)
      actBytes = SerSend(serRef, pRcvBuf, bytes, &err); //02.27.02 GH
    else
      actBytes = SrmSend(serRef, pRcvBuf, bytes, &err); //02.27.02 GH
    MemPtrFree(pRcvBuf);
  }

  if (gOldSerialManager)
    SerSendWait(serRef, bytes * SysTicksPerSecond() / 300); //use lowest speed //02.27.02 GH
  else
    SrmSendWait(serRef); //02.27.02 GH
...
void verb_serialReceive()
{
  ...
  retryReceive:
    ...
    for (i = 0; i < 10; i++)
    {
      ...
      bytes = saveBytes;
      if (gOldSerialManager)
        err = SerReceiveWait(serRef, bytes, timeout); //02.27.02 GH
      else
        err = SrmReceiveWait(serRef, bytes, timeout); //02.27.02 GH

      if ((err == 0) || (err == serErrTimeOut))
        break; //normal case--all chars are in the receive queue or timeout received
      else
      {
        if (err == serErrLineErr)
        {
          if (gOldSerialManager)
            SerReceiveFlush(serRef, SysTicksPerSecond() / 50); //02.27.02 GH
          else
            SrmReceiveFlush(serRef, SysTicksPerSecond() / 50); //02.27.02 GH
        }
      }
    }
    saveErr = err;
  ...
  getIt:
    if (gOldSerialManager)
      err = SerReceiveCheck(serRef, &actBytes); //02.27.02 GH
    else
      err = SrmReceiveCheck(serRef, &actBytes); //02.27.02 GH

    if (actBytes > bytes)
      actBytes = bytes; //limit to our req # chars

    if (gOldSerialManager)
      SerReceive(serRef, pRcvBuf, actBytes, 0, &err); //02.27.02 GH
    else
      SrmReceive(serRef, pRcvBuf, actBytes, 0, &err); //02.27.02 GH

    if (err == 0)
      err = saveErr; //may return timeout error
    setSerErr(err);
  ...
  { //*************************** serial receive with events
    ...
    setSerErr(2); // shouldn't happen

  pRcvBuf[actBytes] = 0x00;
  if (arg1->kind == 's')
    misc_stostr1(pRcvBuf);
  else
  {
    ...
    if (arg2->flags3 & 0x01)
      ; //is  it a constant                      //06012002
    else
      misc_stodbl2(&theBytes); //can only do if not a constant  //06012002
  }
  MemPtrFree(pRcvBuf);
}
...
void FreeListValues(VoidPtr pold, int noItems)
{
  ...
  for (i = 0; i < noItems; ++i)
  {
    ...
    {
      if (StrLen(p) == 0)
        return;       //avoid mem problems --just exit
...
void commonSetScrollbar(char c, FormPtr frmPtr, struct dataelem *arg, double *arg_val)
{
  ...
  if (FrmGetActiveForm() == frmPtr)
    SclDrawScrollBar((ScrollBarPtr)controlPtr);
}
/**************************************************************************************************/
void verb_setscrollbar()
{
...
void commonSetSelected(FormPtr frm, ControlPtr controlPtr, int controlType, int where)
{
  ...
  if (frm == FrmGetActiveForm())
    LstDrawList((ListPtr)controlPtr); //1120
  return;
...
gotTheAssocPopup:
  ...
  if (frm == FrmGetActiveForm())
    CtlDrawControl((ControlPtr)popupObjPtr);
}
/**************************************************************************************************/
...
itsALabel:
  ...
  if (StrLen(text) < MemPtrSize(p))
    MemMove(p, text, StrLen(text));
  else
    MemMove(p, text, MemPtrSize(p) - 1);
...
----*/
    for (i = 0; i < FrmGetNumberOfObjects(frmPtr); ++i)
    {
      ...
      //dbug(debug);
      if (FrmGetObjectId(frmPtr, i) != ctlid)
        continue;
      return (frmPtr);
    }
  }
...
int findControlsPositionOnForm(FormPtr *frmPtr, char *controlname)
{
  ...
  matchedName:
    ...
    DmReleaseResource(h2);
    //dbug("setting result");
    if (FrmGetTitle(*frmPtr) != 0)
      return (i + 1); //must return 1 higher if form has a title
    else
      return (i); //return the control's position
...
void verb_setGeneralProperty()
{
    ...
    else
    {
      if (arg3->kind == 's')
      {
        ...
        if (controlType != frmTableObj) //added 09242003
          ...
          gridSetSelectedText(frmPtr, controlPtr, arg3_val); //added 09242003
      }
      else
        commonErrorWrongType("text");
    }
    break;
  case 1: //status
    if (controlType != frmControlObj)
  ...
  case 3: //maxChars
    ...
    {
      if (arg3->kind != 's')
        commonSetMaxChars(arg3, (double *)arg3_val);
      else
        commonErrorWrongType("maxChars");
    }
    break;
  case 4:  //current
  ...
  case 95: //added 07062005  must allow both 5 and 95
    ...
    else
    {
      ...
      {
        if (arg3->kind != 's')
          commonSetScrollbar(0x02, frmPtr, arg3, (double *)arg3_val);
        else
          commonErrorWrongType("min");
      }
    }
    break;
  ...
  case 96: //added 07062005  must allow both 6 and 96
    ...
    else
    {
      ...
      {
        if (arg3->kind != 's')
          commonSetScrollbar(0x03, frmPtr, arg3, (double *)arg3_val);
        else
          commonErrorWrongType("max");
      }
    }
    break;
  case 7:  //pageSize
  case 97: //added 07062005  must allow both 7 and 97
    ...
    else
    {
      ...
      {
        if (arg3->kind != 's')
          commonSetScrollbar(0x04, frmPtr, arg3, (double *)arg3_val);
        else
          commonErrorWrongType("pageSize");
      }
    }
  ...
  case 48: //selectedCol
    if (controlType != frmTableObj)
      commonErrorWrongCtltype("Col", controlType);
    else
    {
      if (arg3->kind != 's')
        gridSetSelectedCol(frmPtr, controlPtr, (double *)arg3_val);
      else
        commonErrorWrongType("Col");
    }
    break;
  /*
        case 49:  //selectedCol
  ...
  case 50: //ColWidth
    if (controlType != frmTableObj)
      commonErrorWrongCtltype("ColWidth", controlType);
    else
    {
      if (arg3->kind != 's')
        gridSetColWidth(frmPtr, controlPtr, (double *)arg3_val);
      else
        commonErrorWrongType("ColWidth");
    }
    break;
  case 51: //selectedRow
    if (controlType != frmTableObj)
      commonErrorWrongCtltype("Row", controlType);
    else
    {
      if (arg3->kind != 's')
        gridSetSelectedRow(frmPtr, controlPtr, (double *)arg3_val);
      else
        commonErrorWrongType("Row");
    }
    break;
  case 52: //Rows
    if (controlType != frmTableObj)
      commonErrorWrongCtltype("Rows", controlType);
    else
    {
      if (arg3->kind != 's')
        gridSetRows(frmPtr, controlPtr, (double *)arg3_val);
      else
        commonErrorWrongType("Rows");
    }
    break;
  /*
        case 53:  //VisibleRows
  ...
  case 54: //cellMatrix
    if (controlType != frmTableObj)
      commonErrorWrongCtltype("cellMatrix", controlType);
    else
    {
      //note:arg3 is row#
      ...
      //SysFatalAlert(debug);
      if (arg3->kind != 's')
        gridSetCellMatrix(frmPtr, controlPtr, arg3_val);
      else
  ...
  case 55: //valueMatrix
    if (controlType != frmTableObj)
      commonErrorWrongCtltype("valueMatrix", controlType);
    else
    {
      if (arg3->kind != 's')
        gridSetValueMatrix(frmPtr, controlPtr, (double *)arg3_val);
      else
        commonErrorWrongType("valueMatrix");
    }
    break;
  case 56: //value
    ...
    else
    {
      ...
      else
      {
        ...
        if (controlType != frmTableObj)
          commonErrorWrongCtltype("Value", controlType);
        else
        {
          if (arg3->kind != 's')
            gridSetSelectedValue(frmPtr, controlPtr, (double *)arg3_val);
          else
            commonErrorWrongType("Value");
        }
      } //07122005
  ...
  case 57: //rowData
    if (controlType != frmTableObj)
      commonErrorWrongCtltype("rowData", controlType);
    else
    {
      if (arg3->kind != 's')
        gridSetRowData(frmPtr, controlPtr, (double *)arg3_val);
      else
        commonErrorWrongType("RowData");
    }
    break;
    //start additions 02062004
  case 58: //toprow
    if (controlType != frmTableObj)
      commonErrorWrongCtltype("topRow", controlType);
    else
    {
      if (arg3->kind != 's')
        gridSetTopRow(frmPtr, controlPtr, (double *)arg3_val);
      else
        commonErrorWrongType("TopRow");
    }
    break;
  ...
  case 59: //fontMatrix
    ...
    {
      if (arg3->kind != 's')
        gridSetFontMatrix(frmPtr, controlPtr, (double *)arg3_val);
      else
        commonErrorWrongType("fontMatrix");
    }
    break;
  case 80: //left
    ...
    if (arg3->kind != 's')
    {
      ...
      commonErrorWrongType("Left"); //modified 07062005
    break;
  case 81: //top
    if (arg3->kind != 's')
    {
      ...
      commonErrorWrongType("Top"); //modified 07062005
    break;
  ...
  case 82: //width
    if (arg3->kind != 's')
    {
      ...
      commonErrorWrongType("Width"); //modified 07062005
    break;
  case 83: //height
    if (arg3->kind != 's')
    {
      ...
      commonErrorWrongType("Height"); //modified 07062005
    break;
...
redraw:
  ...
  if (frmPtr == FrmGetActiveForm())
    LstDrawList((ListPtr)controlPtr); //redraw the list on the screen
}
/********************************************************************************************/
void verb_listdeleteitem()
{
...
void commonAddItem(FormPtr frmPtr, ControlPtr controlPtr, int controlid, struct dataelem *argn, double *argn_val, struct dataelem *args, char *args_val, char redrawChar)
{
  ...
  if (argn == null_ptr)
    where = LstGetNumberOfItems((ListPtr)controlPtr) + 1; //if didn't specify optional position in list at which to add,
                                                          //then, add to end of list
  else
    where = basc_integ_value(argn, (double *)argn_val); //otherwise, get integer # of position to add at
...
popupClear:
  ...
  if (frmPtr == FrmGetActiveForm())
    LstDrawList((ListPtr)controlPtr);
  return;
...
void verb_generalVerb()
{
  ...
  case 'i': //ctlid
    ...
    //dbug(debug);
    if (found == false)
      break; //probably a bitmap that can't use by id
    frmPtr = findFormTheControlIsOn(controlid);
    pos = FrmGetObjectIndex(frmPtr, controlid);
  ...
  case 4: //deleteitem
    ...
    else
    {
      ...
      if (arg3->kind != 's')
      {
        ...
        commonErrorWrongType("deleteitem");
    }
    break;
  case 5: //startCapture
  ...
  case 8: //redraw/refresh ctl
          ...
          //{
    if (found == false)
      break; //may be shift indic that can't redraw
    if (controlType == frmGraffitiStateObj)
      break; //shift indic can't redraw
  ...
  case 70: //addItem
    if (controlType != frmTableObj)
      commonErrorWrongCtltype("Add", controlType);
    else
    {
  ...
  case 71: //delete
    if (controlType != frmTableObj)
      commonErrorWrongCtltype("Delete", controlType);
    else
    {
  ...
  case 72: //colType
    if (controlType != frmTableObj)
      commonErrorWrongCtltype("ColType", controlType);
    else
    {
  ...
  case 73: //bindToDatabase
    if (controlType != frmTableObj)
      commonErrorWrongCtltype("BindToDatabase", controlType);
    else
    {
